...
def set_st_state_vars():
    ...
    response = f"{COGNITO_DOMAIN}/login?client_id={CLIENT_ID}&response_type=code&scope=email+openid&redirect_uri={APP_URI}"
    return response
...
html_button_login = (
    html_css_login
    + f"<a href='{login_link}' class='button-login' target='_self'>f"{login_link}"</a>"
)
html_button_logout = (
    html_css_login
